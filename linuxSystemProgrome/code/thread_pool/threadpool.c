...
*/
struct NJOB{
	void(*func)(void *arg);/*回调函数，每个任务去执行参数*/
...
//
void *thread_cb(void *arg)//线程的回调 传woker参数进来
{
	struct NWORKER* worker = (struct NWORKER*)arg;
	while(1)
	{	
		pthread_mutex_lock(&worker->pool->mtx);//加锁
		while(worker->pool->jobs == NULL)//任务队列为空
		{
			if(worker->terminate == 0) break;//等于1退出
...
int nThreadPoolCreate(nThreadPool *pool,int numberworks)//线程个数numberworks
{
	...
	pthread_mutex_t blank_mutex = PTHREAD_MUTEX_INITTALTZER;//pthread_mutex_t 应该是锁的数据类型
	memcpy(pool->mtx,&blank_mutex,sizeof(pthred_mutex_t));
	pthread_cond_t blank_cond = PTHREAD_COND_INITTALTZER;//pthread_cond_t 应该是条件变量的数据类型
	...
	{	
		...
		worker->pool = pool;
		pthread_create(&worker->id,NULL,thread_cb,worker);//创建线程
...
{/*多线程的增加删除队列都要加锁*/
	pthread_mutex_lock(&pool->mtx);//加锁
