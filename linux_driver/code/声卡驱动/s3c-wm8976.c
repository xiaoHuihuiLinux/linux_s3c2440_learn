...
static void wm8976_write_reg(unsigned char reg, unsigned int data)
{
	int i;
	unsigned long flags;
	//因为WM8976高七位为地址，低9位位数据
	unsigned short val = (reg << 9) | (data & 0x1ff);
	/*设置输出引脚*/
	s3c2410_gpio_setpin(S3C2410_GPB2,1);//L3mode
	s3c2410_gpio_setpin(S3C2410_GPB3,1);//l3data
	s3c2410_gpio_setpin(S3C2410_GPB4,1);//对应L3Clock

	local_irq_save(flags);//开中断 

	for (i = 0; i < 16; i++){
		if (val & (1<<15))
		{	/*根据芯片手册在clok为低发送数据*/
			s3c2410_gpio_setpin(S3C2410_GPB4,0);
			s3c2410_gpio_setpin(S3C2410_GPB3,1);//数据是1
			udelay(1);
			//发完之后延迟clo拉高
		...
		{
			s3c2410_gpio_setpin(S3C2410_GPB4,0);
			s3c2410_gpio_setpin(S3C2410_GPB3,0);//数据是0
			udelay(1);
			s3c2410_gpio_setpin(S3C2410_GPB4,1);			
		}
		val = val << 1;
	}
	//根据手册发完之后CSB/MODE要发一个锁存
	...
	switch (cmd) {
		...
		case SOUND_MIXER_WRITE_VOLUME: /*设置音量*/
			...
			if (ret)
				return ret;
			/*ioctl val 越大值越大
			 *根据1314的寄存器手册 UDA1341 寄存器的值越小声音越大.
			 *所以就让一个值减减去进来的值
			 *UDA1341的音量是先发出“DATA0”后再发出音量就 如下的：
			 *WM8916 52 53寄存器bit[5 0]表示音量
			*/
			...
			//52寄存器的值 bit 8 为1 bit5-0为uda1341_volume
			wm8976_write_reg(52,(1 << 8) | uda1341_volume);//寄存器52 
...
{
	uda1341_volume = 57;//默认值是
	uda1341_boost = 0;
	//参考WM8916寄存器手册向寄存器0写入任何值都是reset
	/* software reset */
	wm8976_write_reg(0, 0);
	//根据芯片手册要先设置寄存器3 然后1 然后其他的剩余的可以百度
	/* OUT2的左/右声道打开
	 * 左/右通道输出混音打开
	 * 左/右DAC打开
	 */
