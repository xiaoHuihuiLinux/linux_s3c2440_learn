...
static int first_drv_open(struct inode *inode, struct file *file)//open操作设置为输出
{
...
static int first_drv_init(void)
{
	...
	gpfcon = (volatile unsigned long*)ioremap(0x56000050,16);//映射物理地址到虚拟地址参数 ：开始地址 长度 
	gpfdat = gpfcon + 1;//data的地址是0x56000054在上述的基础上地址+1 四字节
	return 0;
}
