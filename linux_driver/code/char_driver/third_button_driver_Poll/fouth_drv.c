...
/*中断事件标志，中断服务程序将他置为1 fouth_drv_read将他清0*/
...
static struct file_operations fouth_drv_fops = {
    ...
    .open   =  fouth_drv_open,     
    .read	=   fouth_drv_read,	 //  .read	=   fouth_drv_read 如果将read	 错误写成write 这个实验会疯狂打印
...
{
	major = register_chrdev(0,"fouth_drv",&fouth_drv_fops);//返回主设备号
...
module_init(fouth_drv_init);//修饰入口函数
