...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	/* Search All DM9000 serial NIC *///寻找网卡
	do {
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			/*这个需要去掉否则会判断这个chip_revision不是0x1a会退出*/
...
static int dmfe_open(struct net_device *dev)
{
	...
	if (request_irq(dev->irq,&dmfe_interrupt,IRQF_TRIGGER_RISING,dev->name,dev)) //我们设置了中断触发的irq 同样要设置中断触发的方式上升沿触发 add jHH
		return -EAGAIN;
...
//#ifdef MODULE //因为这里本身带的需要去掉，因为没有定义这个宏
...
{
	volatile unsigned long *bswcon; //0x48000000设置网卡的总线的宽度
	volatile unsigned long *bankcon4;//设置一些时钟。因为我们的片选是NGCS4
	unsigned long val;//保存寄存器的值
	
	iobase = (int)ioremap(0x20000000,1024); //设置iobase，因为为了选中片选 add jhh
	irq = IRQ_EINT7; //网卡芯片里面有也有内存用来存贮数据通过中断来 add jhh
	/*因为是网卡芯片有读写所以读写肯定有一些时序信号的要求，因为我们是cpu发出指令->mmu->网卡*/
	//设置2440的memory control 寄存器
	bswcon =   ioremap(0x48000000,4);
	bankcon4 = ioremap(0x48000014,4);

	/*DW4 [17:16] 00 = 8-bit 01 = 16-bit, 10 = 32-bit 11 = reserved 我们用的是16条地址线*/
	/*WS4[18]0 = WAIT disable 1 = WAIT enable没有等待信号*/
	/*ST4[19] 设置为0*/
	val =  *bswcon;
	val &= ~(0xf << 16);//将这四位清0
	val |=   (1 << 16);//将bit 16 置为1
	*bswcon = val;
	/*
	tacs[14:13]
	Address set-up time before nGCSn 在片选之前，
	多长时间先发出地址信号dm9000c先选和cmd可以同时发出.设置为00
	*/
	/*
	Tcos[12:11]
	Chip selection set-up time before nOE  发出片选后多长时间发出读信号nOE
	DM9000C的T1>=0ns, 
	所以它设为0*/
	/*Tacc[10:8]
	Access cycle 地址周期 读写脉冲长度
	DM9000C的T2>=10ns, 
	所以它设为1, 表示2个hclk周期,hclk=100MHz,就是20ns
	*/
	/*Tcoh[7:6] 当读信号nOE变为高电平时片选信号还要维持多久
