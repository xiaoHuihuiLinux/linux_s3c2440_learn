...
static char * usb_buf;//收到数据的buffer
static dma_addr_t usb_buf_phys;
static struct urb* uk_urb;
static int len;
static void usbmouse_as_key_irq(struct urb *urb)//“USB 主机控制器”接收到数据后usb主机控制器控制cpu产生中断
{
#if 0
	int i;
	static int cnt = 0;
	...
	{	//input_event参数input_dev结构体，呢类事件 ，这类事件的具体事件 ，值
		input_event(uk_dev,EV_KEY,KEY_L,(usb_buf[1] & (1 << 0)) ? 1 :0);//按下时1 释放时0
		input_sync (uk_dev);
	}
	//右键发生变化
	if((pre_val & (1 << 1)) != (usb_buf[1] & (1 << 1)))
	{	//input_event参数input_dev结构体，呢类事件 ，这类事件的具体事件 ，值
		input_event(uk_dev,EV_KEY,KEY_S,(usb_buf[1] & (1 << 1)) ? 1 :0);//按下时1 释放时0
		input_sync (uk_dev);
	}
	//中键发生变化
	if((pre_val & (1 << 2)) != (usb_buf[1] & (1 << 2)))
	{	//input_event参数input_dev结构体，呢类事件 ，这类事件的具体事件 ，值
		input_event(uk_dev,EV_KEY,KEY_ENTER,(usb_buf[1] & (1 << 2)) ? 1 :0);//按下时1 释放时0
		input_sync (uk_dev);
	}
	pre_val = usb_buf[1];//我自己的鼠标是byte1
	/* 重新提交urb */
...
static int usbmouse_as_key_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	*使用urb
	*/
	uk_urb = usb_alloc_urb(0, GFP_KERNEL);//分配USB 请求块
	usb_fill_int_urb(uk_urb, dev, pipe, usb_buf,len,usbmouse_as_key_irq, NULL, endpoint->bInterval);//填充中断类型的urb
	uk_urb->transfer_dma = usb_buf_phys;//USB 主机控制收到USB 设备数据要写到一个内存的物理地址
