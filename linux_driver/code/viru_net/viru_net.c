...
static void emulator_rx_packet(struct sk_buff *skb, struct net_device *dev)
{
	...
	*type = 0; /* 0表示reply */
	...
	ih->check = ip_fast_csum((unsigned char *)ih,ih->ihl);//ip头的校验码
	
	// 构造一个sk_buff 上报数据
	rx_skb = dev_alloc_skb(skb->len + 2);
	skb_reserve(rx_skb, 2); /* align IP on 16B boundary  保留两个字节*/	
	...
	dev->stats.rx_packets++;//模仿假的包数增加
	dev->stats.rx_bytes += skb->len;////模仿假的字节数增加
...
static int virt_net_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	...
	/*我们参考cx89x0的net_send_packet()*/
	/* 对于真实的网卡, 是下面的步骤：把skb里的数据通过网卡发送出去 */
	...
	netif_wake_queue(dev); /* 数据全部发送出去后,唤醒网卡的队列,它在数据发送完成的中断net_interrupt()中唤醒 */
